...
/**Written by Venujan Malaiyandi, BSCP_CS_61_101*/
 ...
 * @param selected_node     The node before the insert.
 * @param value             The value to store in the node.
*/
void insert_after(node selected_node, int value)
{
    ...
    added_node = new_node(value, *list, selected_node, selected_node->next);
    if(selected_node->next != nullptr)
    {
        selected_node->next->prev = added_node;
    }
    else
    {
        list->last = added_node;
...
/**
 * This function, for_reverse_each, iterates over a linked list in reverse order,
 * applying a visitor function to each node.
 * 
 * @param list  The list of nodes to visit
 * @param fn    The function to call
 * 
*/
void for_reverse_each(const linked_list &list, visitor fn)
{
    node current;
    current = list.last;
...
/**
 * This function, reveser_print_all_with_visitor,
 * prints all the nodes of a linked list in reverse order using a visitor pattern.
 * 
 * @param list      The list of nodes to print
 * 
*/
void reveser_print_all_with_visitor(const linked_list &list)
{
    for_reverse_each(list, print_node);
    write_line(".");
}
...
/**
 * The length function calculates the number of nodes in a given linked list."
* @param list      The list to sum the nodes of.
* @return length   The length of the node 
*/
...
int main()
{
    ...
    add_to_start(my_list, 3);
    add_to_start(my_list, 1);
    print_all(my_list);

    //Insert after node
    write_line("TEST: Inset after first node");
    insert_after(my_list.first,17);
    print_all(my_list);

    write_line("TEST: Inset after last node");
    insert_after(my_list.last,23);
    print_all(my_list);

    //Print reverse
    write_line("TEST: Print Reverse");
    ...
    write_line("TEST: Print Reverse With Visitor");
    reveser_print_all_with_visitor(my_list);
